...
cpuWFR::cpuWFR(int imgwidth, int imgheight, std::array<int, 4> ROI, int sigmax,
  ...
  for (auto &i : ParallelPool) {
    i.freq_handle_in =
        (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * mm * nn);
    i.freq_handle_out =
        (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * mm * nn);
    i.ifreq_handle_in =
        (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * mm * nn);
    ...
    i.freq_plan = fftwf_plan_dft_2d(mm, nn, i.freq_handle_in, i.freq_handle_out,
                                    FFTW_FORWARD, FFTW_ESTIMATE);
    i.ifreq_plan =
        fftwf_plan_dft_2d(mm, nn, i.ifreq_handle_in, i.ifreq_handle_out,
  ...
  pre_plan = fftwf_plan_dft_2d(mm, nn, pre_handle_in, pre_handle_out,
                               FFTW_FORWARD, FFTW_ESTIMATE);
  ...
  // 计算窗口中心坐标（从0开始索引）
  float center_x = (cal_width - 1) / 2.0;
  float center_y = (cal_height - 1) / 2.0;

  // 预计算常数
  const float sigma_x_sq = static_cast<float>(sigmax) * sigmax;
  const float sigma_y_sq = static_cast<float>(sigmay) * sigmay;
  const float two_sigma_x_sq = 2.0 * sigma_x_sq;
  const float two_sigma_y_sq = 2.0 * sigma_y_sq;
  ...
  for (int y = 0; y < cal_height; ++y) {
    ...
    const float y_term = dy_sq / two_sigma_y_sq;

    for (int x = 0; x < cal_width; ++x) {
      const float dx = static_cast<float>(x) - center_x;
      const float dx_sq = dx * dx;
      const float x_term = dx_sq / two_sigma_x_sq;

      // 计算高斯值：exp(-(dx²/(2σx²) + dy²/(2σy²)))
      const float exponent = -(x_term + y_term);
      const float gaussian_value = std::exp(exponent);
  ...
  float norm_factor = std::sqrt(sum_of_squares);
...
cpuWFR::~cpuWFR() {
  fftwf_destroy_plan(pre_plan);

  fftwf_free(pre_handle_in);
  fftwf_free(pre_handle_out);

  for (auto &i : ParallelPool) {
    fftwf_destroy_plan(i.freq_plan);
    fftwf_destroy_plan(i.ifreq_plan);
    fftwf_free(i.ifreq_handle_in);
    fftwf_free(i.ifreq_handle_out);
    fftwf_free(i.freq_handle_in);
    fftwf_free(i.freq_handle_out);
  }
}

std::vector<float> cpuWFR::operator()(std::vector<unsigned char> image) {
  memset(pre_handle_in, 0, sizeof(fftwf_complex) * mm * nn);
...
#pragma omp parallel for
  ...
  // 此时，列优先
  fftwf_execute(pre_plan);

  memset(result_ridge.data(), 0, sizeof(float) * ROI[2] * ROI[3]);
...
#pragma omp parallel for
  for (int freq_index = 0; freq_index < freqListSize; freq_index++) {
    ...
    memset(fft_obj.freq_handle_in, 0, sizeof(float) * mm * nn * 2);
    ...
    fftwf_execute(fft_obj.freq_plan);
    ...
    fftwf_execute(fft_obj.ifreq_plan);
...
#pragma omp critical
  ...
  std::vector<float> result(result_ridge.size());
  // 这里转回行优先
#pragma omp parallel for
  for (int i = 0; i < ROI[2]; i++)
    for (int j = 0; j < ROI[3]; j++)
